...
基于最近一轮真实跑数和 notebook 验证，当前阶段不再继续主动推进建模相关迭代。执行口径调整为：

1. 先把最开始的分析功能做稳：`profile_stats`、`profile_risk`、`MarsBinEvaluator`、`MarsBinningReport`、`MarsStatsSelector`、`reporting` 图表与导出。
2. 先修口径一致性，再扩业务能力：优先解决 summary / detail / trend / plot / multi-target 之间的语义不一致。
3. 暂停 `modeling / pipeline / scorecard / replay / artifact` 侧的新能力建设和系统性重构。
4. 建模侧只接受阻塞性修复：若问题不直接阻塞分析层主链路验证，就不在当前阶段展开。
...
- 但在当前阶段，这项工作已暂停主动推进；只有当 modeling 代码直接阻塞分析层修复时，才允许做最小修补。
...
5. `P1-04` 先补测试骨架，优先保护分析层回归
6. `P1-05` + `P1-12` + `P1-13` 做工程质量、性能基准、异常收口
7. `P1-14` 收口稳定 / internal / experimental API 边界
8. `P1-02` + `P1-07` 做配置治理和全局配置中心
9. `P1-09` 收口统一物化策略
10. `P1-01` sklearn 协议补齐
11. `P1-17` 缺失率异常检测
12. `P1-11` API 兼容与废弃策略
13. 分析层稳定后，再恢复 `P1-18`
14. 分析层稳定后，再恢复 `P1-03`
15. 分析层稳定后，再恢复 `P1-08` + `P1-10` + `P1-15` + `P1-16`
16. 最后再看 `P1-06` Vintage / Swap / 金额纵向分析
17. `P2` 体验与中期增强项
