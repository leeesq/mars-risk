...
def _build_profile_mapping_frame(binner: MarsBinnerBase) -> pl.DataFrame:
    """把分箱标签映射缓存转换为表现表可 join 的长表。"""
...
def _build_transform_output_expr(
    ...
    *,
    ...
    str_map = {str(key): value for key, value in binner.bin_mappings_.get(col, {}).items()}
