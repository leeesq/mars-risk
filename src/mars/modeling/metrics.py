...
def normalize_metric_directions(
    ...
    for metric_name in metric_names:
        ...
        raw_direction = str(
            user_directions.get(normalized_name, DEFAULT_METRIC_DIRECTIONS.get(normalized_name, "maximize"))
        ).lower()
...
def evaluate_metric(
    ...
    *,
    ...
    return round(float(custom_metric(np.asarray(y_true).reshape(-1), as_probability(y_pred))), 6)
