...
class MarsFeatureIncrementalTuner:
    ...
    def _success_row(
        ...
        *,
        ...
        direction = dict(getattr(run, "metric_directions", {}) or {}).get(
            selection_metric,
            "maximize",
        )
        ...
        row["selection_score"] = pd.to_numeric(
            pd.Series([history_row.get(score_col)]),
            errors="coerce",
        ).iloc[0]
    ...
    def _choose_best(
        ...
        runs: Mapping[int, MarsModelTuningResult],
        metric: str,
    ) -> tuple[int | None, MarsModelTuningResult | None]:
        ...
        if sample_run is not None:
            direction = dict(getattr(sample_run, "metric_directions", {}) or {}).get(
                metric,
                "maximize",
            )
        best_idx = (
    ...
    def tune(
        ...
        **tune_kwargs : Any
        ...
        for feature_count in resolved_steps:
            ...
            step_cats = [
                feature
                for feature in self.spec.categorical_features
                if feature in step_features
            ]
