---
description: 安装 MARS 0.0.26、Python 3.8 兼容栈、可选依赖和本地开发环境。
---

# 安装

MARS `0.0.26` 的基础包支持 Python 3.8–3.12，Python 3.13+ 暂不开放安装。
...
!!! warning "发布前提"
    ...
    正式站点只应在 PyPI 已发布 `0.0.26` 后部署。发布前请从源码安装进行预览验收。
...
| 场景 | 安装命令 |
| --- | --- |
| Notebook | `pip install "mars-risk[notebook]==0.0.26"` |
| 树模型 | `pip install "mars-risk[ml]==0.0.26"` |
| 调参 | `pip install "mars-risk[ml,tuning]==0.0.26"` |
| 文档构建 | `pip install "mars-risk[docs]==0.0.26"` |
...
输出应为 `0.0.26`。随后运行[10 分钟 Quickstart](quickstart.md)。
