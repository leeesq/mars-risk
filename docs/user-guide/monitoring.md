...
`mars.monitoring` 提供特征与模型分的通用监控指标计算层。模型分、prob、score 本质上可以视为特殊特征，因此特征监控和模型监控可以共用同一套计算逻辑。

MARS 不负责调度、模型注册、线上看板和业务处置流程。MARS 负责计算结构化指标和默认报警摘要；监控窗口、基准样本、模型版本、阈值策略和业务处置由使用者定义。

## 支持场景

- **前端监控**：例如申请入口、渠道、产品、地区等在进入模型前的特征分布、缺失率和分箱占比变化。
- **后端监控**：例如模型分、prob、score、表现期 target 覆盖率和已表现样本风险指标。
